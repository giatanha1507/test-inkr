...
const data = {
	tags: ['Horror', 'Psychological', 'Seinen'],
	summary: {
	...
	return (
		<div className={styles['card-background']}>
			...
			<div style={{ paddingBottom: 20 }}>
				<p className={styles['card-text']} style={{ paddingBottom: 8 }}>
					Genres
				</p>
				<div style={{ display: 'flex' }}>
					{data.tags.map((tag, index) => (
						<Tag className={styles['tag-text']} key={index}>
							{tag}
						</Tag>
					))}
				</div>
			...
			{show ? (
				...
				<div>
					<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 20 }}>
						{data.summary.img.map((img, index) => (
							<Image
					...
					<div style={{ paddingBottom: 20 }}>
						...
						{data.credits.map((credits) => (
							<div
								key={credits.id}
								style={{ display: 'flex', marginBottom: 14, alignItems: 'center' }}
							>
								{credits.ava ? (
									<Avatar
										style={{ width: 40, height: 40, marginRight: 15 }}
										src={<Image preview={false} src={credits.ava} style={{ width: 40 }} />}
									/>
								) : (
									<Avatar
										style={{ width: 40, height: 40, marginRight: 15, backgroundColor: '#87d068' }}
										icon={<UserOutlined />}
									/>
								)}
								<div>
					...
					<div>
						...
						{data.facts.map((facts) => (
							<div key={facts.info} style={{ display: 'flex', alignItems: 'flex-end' }}>
								...
								<p className={styles['card-desc']} style={{ color: 'rgba(255, 255, 255, 0.85)' }}>
									{facts.content}
								</p>
							</div>
						))}
